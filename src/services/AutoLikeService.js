...
class AutoLikeService {
  ...
  async assignInitialLikesToPost(postId) {
    try {
      ...
      console.log(
        `✅ Assigned ${likesCreated} initial likes to post ${postId}`
      );
      return likesCreated;
    } catch (error) {
      console.error(
      ...
      for (const post of recentPosts) {
        try {
          ...
          post.engagementMetrics.likes =
            (post.engagementMetrics.likes || 0) + randomLikeCount;
          await post.save();
          ...
          // }
          totalLikesAdded += randomLikeCount;
          postsUpdated++;
          console.log(
            `✅ Added ${randomLikeCount} likes to post "${post.title.substring(0, 40)}..."`
          );
        } catch (postError) {
          console.error(
    ...
    try {
      ...
      for (const post of posts) {
        try {
          ...
          console.error(`Error recalculating post ${post._id}:`, error.message);
        }
      }
